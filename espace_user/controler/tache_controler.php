<?php
    ...
    class tache_controler{
        ...
        public function update_tache_page($id_tache){
            if($_SESSION["loged_in"] == "admin") {
                $tache=new tache_transaction;
                $user=new user_transaction;
                $add_membres=$user->add_memebres_toproject();
        ...
        public function update_tache($id_tache){
            if($_SESSION["loged_in"] == "admin") {
                $tache=new tache_transaction;
                $statut_tache=$_POST["statut_tache"];
                $id_membre=$_SESSION["id_membre"];
                $tache->update_tache($id_membre,$id_tache,$statut_tache);
                header("location:index.php?action=display_taches&id_membre=$id_membre");
            } 
            else{
                header("location:index.php");
            }
        ...
        public function afficher_taches($id_membre){
            if($_SESSION["loged_in"] == "admin") {
                $tache =new tache_transaction;
                $taches=$tache->afficher_taches($id_membre);
                include_once "vue/html/afficher_tache.php";
            } 
            else{
                header("location:index.php");
            }
